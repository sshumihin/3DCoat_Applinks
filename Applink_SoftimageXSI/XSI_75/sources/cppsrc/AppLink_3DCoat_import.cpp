...
CStatus BuildMaterials()
{
	...
	if(Get3DCoatParam(L"bImpMat").GetValue())
	{
		...
		for (ULONG n=0; n < vmatCollection.size(); n++ )
		{
			Material myMat;
...
bool IsShaderConnected(Shader& lShader, const CString p)
{
	...
	for( long k=0; k < lImages.GetCount(); k++ )
	{
		...
		if ( lTargetIm.GetName().IsEqualNoCase( p ) )
		{
			isConnected = true;
			break;
		}
...
bool IsShaderConnected(Material& myMat, const CString p)
{
	bool isConnected = false;

	CRefArray lImages = myMat.GetShaders();
	for( long k=0; k < lImages.GetCount(); k++ )
	{
		Shader lImage(lImages[ k ]);
		Parameter lTargetIm = lImage.GetShaderParameterTargets( L"out" )[ 0 ];
...
void CreateShaders(Material& myMat, ULONG n)
{
	...
	if(strImagePath != L"")
	{
		if(!IsShaderConnected(sh, L"diffuse"))
		{
			CValueArray args(1);
	...
	if(strImagePath != L"")
	{
		if(!IsShaderConnected(sh, L"specular"))
		{
			CValueArray args(1);
			CValue valImageClip2;
			args[0] = strImagePath ;
	...
	if(strImagePath != L"")
	{
		if(!IsShaderConnected(myMat, L"displacement"))
		{
			...
			CRef newSource;
			CRef prevSource;
			CStatus st = disp.ConnectFromPreset(L"Image", siTextureShaderFamily, prevSource, newSource);
	...
	if(strImagePath != L"")
	{
		if(!IsShaderConnected(myMat, L"normal"))
		{
			...
			CRef newSource;
			CRef prevSource;
			CStatus st = normal.ConnectFromProgID(L"Softimage.XSINormalMap2.1", prevSource, newSource);
			Shader nnormal = newSource;

			Parameter map = nnormal.GetParameters().GetItem(L"Map");

			st = map.ConnectFromProgID(L"Softimage.sib_color_combine.1", prevSource, newSource);
			Shader combine = newSource;

			Parameter red = combine.GetParameters().GetItem(L"red");
			st = red.ConnectFromProgID(L"Softimage.sib_channel_picker.1", prevSource, newSource);
			...
			st = green.ConnectFromProgID(L"Softimage.sib_channel_picker.1", prevSource, newSource);
			...
			st = blue.ConnectFromProgID(L"Softimage.sib_channel_picker.1", prevSource, newSource);
...
XSIPLUGINCALLBACK CStatus  Coat3DImport_Init( CRef& in_ctxt )
{
...
XSIPLUGINCALLBACK CStatus  Coat3DImport_Execute( CRef& in_ctxt )
{
