...
void CreateMaterial(Scene& oScene, CString mtlName, Material& myMat, ULONG lm)
{
	MaterialLibrary matlib( oScene.GetActiveMaterialLibrary( ) );
	myMat = matlib.CreateMaterial( L"Phong", mtlName);
	CreateShaders(myMat, lm);
}

Material FindMaterial(CRefArray mtlLibs, CString& mtlName)
{
	for ( LONG m=0; m < mtlLibs.GetCount(); m++ )
	{
		MaterialLibrary oMatlib = mtlLibs[m];
		CRefArray nbMaterials = oMatlib.GetItems();

		for ( LONG n=0; n < nbMaterials.GetCount(); n++ )
		{
			Material oMat = nbMaterials[n];
			if(oMat.GetName() == mtlName)
			{
				return oMat;
				break;
			}
...
CStatus BuildMaterials()
{
	...
	if(Get3DCoatParam(L"bImpMat").GetValue())
	{
		...
		for (unsigned int n=0; n < vmatCollection.size(); n++ )
		{
			Material myMat;
...
CStatus BuildObjects()
{
	...
	for(int i=0; i<GroupNames.GetCount(); i++ )
	{
		...
		if(Get3DCoatParam(L"bImpMat").GetValue())
		{
			bool bReplace = Get3DCoatParam(L"bReplace").GetValue();
			...
			if(vpolyClusters.size() > 1)
			{
				for(ULONG lm=0; lm < vpolyClusters.size(); lm++)
				{
					...
					if(vpolyClusters[lm].GetCount() > 0)
					{
						...
						if(bReplace)
						{
							myMat = FindMaterial(oMatlibs, MaterialNames[lm]);
							if(!myMat.IsValid())
							{
								CreateMaterial(oScene, MaterialNames[lm], myMat, lm);
							}
							CreateShaders(myMat, lm);
						}
						else
			...
			{
				Material myMat;
				if(vpolyClusters[0].GetCount() > 0)
				{
					if(bReplace)
					{
						myMat = FindMaterial(oMatlibs, MaterialNames[0]);
						if(!myMat.IsValid())
						{
							CreateMaterial(oScene, MaterialNames[0], myMat, 0);
						}
						CreateShaders(myMat, 0);
					}
					else
					{
						CreateMaterial(oScene, MaterialNames[0], myMat, 0);
					}
					xobj.SetMaterial(myMat);
				}				
			}
		}
