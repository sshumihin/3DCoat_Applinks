...
SICALLBACK AppLink_3DCoat_Define( CRef& in_ctxt )
{
	...
	pset.AddParameter(L"exeLocation", CValue::siString, siPersistable, L"3DCoat.exe", L"", dft, param);
	...
	pset.AddParameter(L"bStart", CValue::siBool, NULL, L"Start 3DCoat", L"", true, param);
...
{	
	...
	CString txt = L"*Select folder ..\\My Documents\\3D-CoatV49\\Exchange";
	ppg.Clear();
...
{	
	...
	if ( eventID == PPGEventContext::siOnInit )
	{
		...
		if(prop.GetParameter(L"coatLocation").GetValue() == L"")
		{
			...
			{
				CString exchPath = CUtils::BuildPath(Path, L"3D-CoatV49", L"Exchange");
		...
		if(prop.GetParameter(L"exeLocation").GetValue() == L"")
		{
			...
			{
				CString exePath = CUtils::BuildPath(Path, L"3D-Coat-V4", L"3DCoatGL64.exe");
				prop.GetParameter(L"exeLocation").PutValue(exePath);
			}
		}
				...
				if(bStart)
				{
					...
					if(exefile.good())
					{
						...
						if(!Find3DCoat())
						{
							...
							{
								app.LogMessage(L"3DCoat.exe not found! See '3DCoat.exe' field in the Property Page.", siWarningMsg);
							}
						}
						else
						{
							app.LogMessage(L"3DCoat.exe is run!", siWarningMsg);
						}
					}
					else
					{
