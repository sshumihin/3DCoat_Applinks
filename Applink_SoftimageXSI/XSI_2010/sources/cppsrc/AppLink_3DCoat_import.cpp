...
void CreateShaders(Material& myMat, ULONG n)
{
	...
	CRef newSource;
	CRef prevSource;
	surf.ConnectFromProgID(L"Softimage.material-phong.1", prevSource, newSource);
	Shader sh = newSource;
	...
	if( swmap.GetAsText() == L"1")
	{
		...
		if(strImagePath != L"")
		{
			...
			CRef newSource;
			CRef prevSource;
			CStatus st = disp.ConnectFromProgID(L"Softimage.sib_zbump.1", prevSource, newSource);
	...
	if(swmap.GetAsText() == L"0")
	{
		...
		if(strImagePath != L"")
		{
			...
			CRef newSource;
			CRef prevSource;
			CStatus st = normal.ConnectFromProgID(L"Softimage.XSINormalMap3.1", prevSource, newSource);
