...
SICALLBACK OutputImportTxt()
{
	...
	if (tfw.is_open())
	{
		CString objname = ((X3DObject)app.GetSelection().GetItem(0)).GetName();
		CString objTmp = CUtils::BuildPath(Get3DCoatParam( L"tempLocation" ).GetValue(), objname, objname + L".obj" );
...
SICALLBACK Coat3DExport_Execute( CRef& in_ctxt )
{
...

	{
		...
		CString objname = ((X3DObject)selection.GetItem(0)).GetName();
		CString strOut = CUtils::BuildPath(Get3DCoatParam( L"tempLocation" ).GetValue(), objname, objname + L".obj" );
		
		if(!CUtils::EnsureFolderExists(strOut, true))
		{
			CString str = L"File "+strOut+" is not be writed!\nMake sure that the user permissions allow write file.";
			app.LogMessage(str, siErrorMsg);
			return CStatus::False;
