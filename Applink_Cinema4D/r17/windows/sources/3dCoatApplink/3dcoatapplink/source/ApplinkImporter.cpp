...
Bool ApplinkImporter::gatheringObjDate(char* path)
{
	...
	GePrint("Nb Grp: " + String::IntToString(numGroups));
	for(unsigned int i = 0; i < groups.size(); i++)
	{
		groups[i].faces = new Face[groups[i].numGFaces + 1];
		groups[i].polyMatIdx = new int[groups[i].numGFaces + 1];
		GePrint("Nb faces: " + String::IntToString((Int32)groups[i].numGFaces));
...
Bool ApplinkImporter::Execute(BaseDocument* document, BaseContainer* bc)
{
	...
	this->pSet.impComboMap = bc->GetInt32(IDC_COMBO_MAP_IMPORT);
...
Bool ApplinkImporter::readMTL()
{
	...
	Int32 length = smtlFilePath.GetCStringLen(STRINGENCODING_8BIT) + 1;
...
//////////////////////////////////////////////
	...
	buffer = NewMemClear(char, length);
