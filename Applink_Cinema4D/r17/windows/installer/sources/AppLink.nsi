...
DirText "Please choose the folder where Maxon Cinema 4D R17 and above is installed to copy 3D-Coat AppLink plugin files there."  
...
; The file to write
OutFile "ApplinkCinema4DR17win_v1.0.exe"
...
;InstallDir "$INSTDIR\MAXON\CINEMA 4D R17\plugins"

; IMPORTANT: or you may get it from come registry key if need, if there is no nregistery key, the one above will be used
;InstallDirRegKey HKCU "Software\3D-Coat-V45" ""

Function dirImage
	!insertmacro BIMAGE "C4DR17-header.bmp" /RESIZETOFIT
FunctionEnd

Function .onInit
	
	${If} ${RunningX64}
	StrCpy $INSTDIR "$PROGRAMFILES64\MAXON\CINEMA 4D R17\plugins"
	${Else}
	StrCpy $INSTDIR "$PROGRAMFILES32\MAXON\CINEMA 4D R17\plugins"
	${EndIf}
	
  	InitPluginsDir
	# the plugins dir is automatically deleted when the installer exits
	InitPluginsDir
	File /oname=$PLUGINSDIR\splash.bmp "3DC-V45-loading.bmp"
