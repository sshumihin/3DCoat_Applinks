...
DirText "Please choose the folder where Maxon Cinema 4D R14 and above is installed to copy 3D-Coat AppLink plugin files there."  
...
; The file to write
OutFile "ApplinkCinema4DR14win_v1.0.exe"
...
;InstallDir "$INSTDIR\MAXON\CINEMA 4D R14\plugins"
...
Function dirImage
	!insertmacro BIMAGE "C4DR14-header.bmp" /RESIZETOFIT
FunctionEnd

Function .onInit
	${If} ${RunningX64}
	StrCpy $INSTDIR "$PROGRAMFILES64\MAXON\CINEMA 4D R14\plugins"
	${Else}
	StrCpy $INSTDIR "$PROGRAMFILES32\MAXON\CINEMA 4D R14\plugins"
	${EndIf}
