...
Bool ApplinkImporter::parseObjFile(char* path)
{
	...
	while(!fsImportObj.eof())
	{
		...
		else if(!strncmp("v ", line, 2))
		{
			...
			//GePrint("vert " + LongToString(pointIndex) + ": " + RealToString(x) + ", " + RealToString(y) + ", " + RealToString(z));
			verticies[pointIndex].x = Real(x);
			verticies[pointIndex].y = Real(y);
			verticies[pointIndex].z = Real(z);
			//GePrint("vert " + LongToString(pointIndex) + ": " + RealToString(verticies[pointIndex].x) + ", " + RealToString(verticies[pointIndex].y) + ", " + RealToString(verticies[pointIndex].z));
			pointIndex++;
		}
		else if(!strncmp("vt", line, 2))
		{
			double x, y, z;
			sscanf(line, "vt %lf %lf", &x, &y);
			sscanf(line, "vt %lf %lf %lf", &x, &y, &z);
			this->uvw[texCoordIndex].x = Real(x);
			this->uvw[texCoordIndex].y = Real(1 - y);
			this->uvw[texCoordIndex].z = Real(z);
		...
		else if(!strncmp("f ", line, 2))
		{
			...
			subArray.clear();
			for(ULONG i = 1; i < stringArray.size(); i++)
			{
				subArray = this->Split(stringArray[i], '/');
				//GePrint("0: " + String(subArray[0].c_str()));
				//GePrint("1: " + String(subArray[1].c_str()));
				//GePrint("2: " + String(subArray[2].c_str()));
...
/////////////////////////////////
	...
	this->exportFilePath.GetString().GetCString(file, 255, StXbit);
...
Bool ApplinkImporter::readMTL()
{
	...
	while(!fsImportMtl.eof())
	{
		...
		{
			sscanf(line, "Ns %lf", &d);
			this->matArray[materialIndex].Ns = Real(d);
		}
		else if(!strncmp("d ", line, 2))
		{
			sscanf(line, "d %lf", &d);
			this->matArray[materialIndex].d = Real(d);
		}
		...
		{
			sscanf(line, "Kd %lf %lf %lf", &x, &y, &z);
			this->matArray[materialIndex].Kd.x = Real(x);
			this->matArray[materialIndex].Kd.y = Real(y);
			this->matArray[materialIndex].Kd.z = Real(z);
		}
		else if(!strncmp("Ka ", line, 3))
		{
			sscanf(line, "Ka %lf %lf %lf", &x, &y, &z);
			this->matArray[materialIndex].Ka.x = Real(x);
			this->matArray[materialIndex].Ka.y = Real(y);
			this->matArray[materialIndex].Ka.z = Real(z);
		}
		else if(!strncmp("Ks ", line, 3))
		{
			sscanf(line, "Ks %lf %lf %lf", &x, &y, &z);
			this->matArray[materialIndex].Ks.x = Real(x);
			this->matArray[materialIndex].Ks.y = Real(y);
			this->matArray[materialIndex].Ks.z = Real(z);
		}
		else if(!strncmp("Ke ", line, 3))
		{
			sscanf(line, "Ke %lf %lf %lf", &x, &y, &z);
			this->matArray[materialIndex].Ke.x = Real(x);
			this->matArray[materialIndex].Ke.y = Real(y);
			this->matArray[materialIndex].Ke.z = Real(z);
		}
...
//////////////////////////////////////////////
	...
	length = textFilePath.GetCStringLen(St8bit) + 1;
	buffer = (CHAR*)GeAlloc(length);
	textFilePath.GetCString(buffer, length, St8bit);
	...
	GePrint("File " + this->texturesFilePath.GetString() + " open!");
...
Bool ApplinkImporter::CreateChannelSpecular(long i, BaseMaterial* mat)
{
	if(this->matArray[i].map_Ks != "")
	{
		...
		GeData b;
		mat->GetParameter(DescID(MATERIAL_USE_SPECULARCOLOR), b, 0);
		if(b == FALSE) mat->SetParameter(DescID(MATERIAL_USE_SPECULARCOLOR), TRUE, 0);
		BaseContainer cdata = color->GetData();
...
Bool ApplinkImporter::CreateChannelDisplacement(long i, BaseMaterial* mat)
{
	if(this->matArray[i].disp_map != "")
	{
		...
		GeData b;
		mat->GetParameter(DescID(MATERIAL_USE_DISPLACEMENT), b, 0);
		if(b == FALSE)	mat->SetParameter(DescID(MATERIAL_USE_DISPLACEMENT), TRUE, 0);
		BaseContainer cdata = color->GetData();
		//GePrint("disp: " + RealToString(this->matArray[i].disp_k));
		mat->SetParameter(DescID(MATERIAL_DISPLACEMENT_STRENGTH), this->matArray[i].disp_k/100, 0);
...
Bool ApplinkImporter::CreateChannelNormal(long i, BaseMaterial* mat)
{
	if(this->matArray[i].norm_map != "")
	{
		...
		if (!color)	return false;
		GeData b;
		mat->GetParameter(DescID(MATERIAL_USE_NORMAL), b, 0);
		if(b == FALSE) mat->SetParameter(DescID(MATERIAL_USE_NORMAL), TRUE, 0);
		BaseContainer cdata = color->GetData();
		mat->SetParameter(DescID(MATERIAL_NORMAL_REVERSEY), TRUE, 0);
