...
//////////////////////////////
				if(bc->GetBool(IDC_CHK_EXP_UV))
				{
					...
					else
					{
						...
						const void* dataptr = uvw_tag->GetDataAddressR();
						UVWStruct res;
						
						for(LONG t=0, y=0; t < pcnt; t++)
						{
							//GePrint("y: " + LongToString(y));
							uvw_tag->Get(dataptr, t, res);
...
void ApplinkExporter::WriteVertexPositions(BaseFile* objfile, ExportObject& mObject, LONG vcnt)
{
	...
	{
		str = "v " + RealToString(mObject.Vp[p].x, -1, 6) + " " + RealToString(mObject.Vp[p].y, -1, 6) + " " + RealToString(mObject.Vp[p].z, -1, 6) + "\n";
		this->WriteString(str, objfile);
	}
...
/*	for(t = 0; t < pcnt*4; t++)
	...
	for(LONG t = 0, y=0; t < pcnt; t++)
	{							
		str = "vt " + RealToString(mObject.Vt[y].x, -1, 6) + " " + RealToString( 1 - mObject.Vt[y].y, -1, 6) + " " + RealToString(mObject.Vt[y].z, -1, 6) + "\n";
		this->WriteString(str, objfile);
		str = "vt " + RealToString(mObject.Vt[y+1].x, -1, 6) + " " + RealToString(1 - mObject.Vt[y+1].y, -1, 6) + " " + RealToString(mObject.Vt[y+1].z, -1, 6) + "\n";
		this->WriteString(str, objfile);
		str = "vt " + RealToString(mObject.Vt[y+2].x, -1, 6) + " " + RealToString(1 - mObject.Vt[y+2].y, -1, 6) + " " + RealToString(mObject.Vt[y+2].z, -1, 6) + "\n";
		this->WriteString(str, objfile);
		if(mObject.Fpvnb[t] == 4)
		{
			str = "vt " + RealToString(mObject.Vt[y+3].x, -1, 6) + " " + RealToString(1 - mObject.Vt[y+3].y, -1, 6) + " " + RealToString(mObject.Vt[y+3].z, -1, 6) + "\n";
			this->WriteString(str, objfile);
		}
		y += mObject.Fpvnb[t];
	}
...
void ApplinkExporter::ExportChannel(BaseDocument* document, BaseFile* file, BaseMaterial& material, LONG shaderId, LONG colorId, const String s)
{
	...
	str += RealToString(color.x, -1, 6) + " " + RealToString(color.y, -1, 6) + " " + RealToString(color.z, -1, 6) + "\n";
